Page({
  ...
  onRefresherRefresh() {
    setTimeout(() => {
      ...
      this.selectComponent('.scroll-view').stopRefresher();
    ...
    onLoad() {
      ...
      this.init()
      // const urls = getApp().globalData.myGlobalData.map(item => item.img);
    ...
    init() {
        ...
        const newAll = (this.data.all.concat(this.data.first)).map(item => {
            // const newAll = (this.data.first.map(item => {
            const da = getApp().globalData.myGlobalData.filter(subItem => item.nav.includes(subItem.title))
            // const da = getApp().globalData.myGlobalData.filter(subItem => item.nav.includes(Number(subItem.id)));
            const sortedDa = item.nav.map(title => da.find(subItem => subItem.title == title)).filter(Boolean);
            // console.warn(item.title)
            // console.warn(da.map(item => `"${item.title}"`).join(', '))
            return {
                ...item,
                navList: sortedDa
            }
        ...
        // console.log(`方法执行耗时: ${duration} 毫秒`);
        this.setData({
            all: this.data.first.concat(newAll),
            // displayedList: newAll.slice(0, 7)
        });

        // wx.getImageInfo({
        //   src: "https://img0.baidu.com/it/u=3800788057,3822337833&fm=253&fmt=auto&app=120&f=JPEG?w=750&h=500",
        //   success: (res) => {
        //     const path = res.path;
        //     const ctx = wx.createCanvasContext('myCanvas');
        //     ctx.drawImage(path, 0, 0, 120, 85);
        //     ctx.draw();
        //   },
        //   fail: (err) => {
        ...
        // all.forEach((outerItem, outerIndex) => {
        //   outerItem.navList.forEach((innerItem, innerIndex) => {
        //     console.warn("innerItem.img:"+innerItem.img)
        //     // const canvasId = `canvas-${outerIndex}-${innerIndex}`;
        ...
        // console.log(`方法执行耗时: ${duration2} 毫秒`);
        // const that = this;
        wx.getLocation({
            type: 'gcj02',
            success(res) {
    ...
    loadDataInBatches(data, startIndex) {
        const batchSize = 5;
        const {displayedList} = this.data;
        const endIndex = startIndex + batchSize;
        const batch = data.slice(startIndex, endIndex);

        if (batch.length > 0) {
            this.setData({
                displayedList: displayedList.concat(batch)
            }, () => {
                // 数据更新完成后，继续加载下一批
            ...
            this.setData({
                displayedList: displayedList.concat(this.data.last)
            });
        }
    },
    // onReachBottom2() {
    //   const { all, displayedList } = this.data;
    //   const currentLength = displayedList.length;
    //   const nextBatch = all.slice(currentLength, currentLength + 8);
    //   if (nextBatch.length > 0) {
    //     this.setData({
    //       displayedList: displayedList.concat(nextBatch)
    //     });
    //   }
    // },
    onPlaceTap(e) {
    ...
    goToMap(e) {
        ...
        if (poi.nav.length > 0) {
            getApp().globalData.source = 'cd';
            getApp().globalData.filterItem = poi.ifs;
            getApp().globalData.navIds = poi.nav;
            wx.switchTab({
                url: `/pages/map/map`,
                success: function (res) {
                    console.log('跳转成功');
    ...
    onImageLoad(e) {
        ...
        // console.warn( index1)
        // const items = this.data.displayedList;
        // items[index1].navList[index].isLoaded = true; // 标记该图片已加载完成
        // this.setData({
        //   displayedList:items
        // });
        // console.warn( items[index1])
    },
    onImageError(e) {
        // const { index, index1 } = e.currentTarget.dataset;
        // const items = this.data.displayedList;
        // items[index1].navList[index].isLoaded = false; // 标记该图片已加载完成
        // this.setData({
        //   displayedList:items
        // });
    },
    ...
    initAllImages: function () {
        const all = this.data.all;
        all.forEach((outerItem, outerIndex) => {
            const navList = outerItem.navList;
            navList.forEach((innerItem, innerIndex) => {
                this.processImage(innerItem.img, outerIndex, innerIndex);
            });
        });
    },
    processImage: async function (imageUrl, outerIndex, innerIndex) {
        try {
            // 获取图片信息
            const res = await this.getImageInfo(imageUrl);
            ...
            // 绘制压缩后的图片
            ctx.drawImage(res.path, 0, 0, newWidth, newHeight);
            await this.drawCanvas(ctx, canvasId);

            // 将 canvas 内容保存为临时文件
            const tempRes = await this.canvasToTempFilePath(canvasId);
            const compressedPath = tempRes.tempFilePath;

            // 更新数据
            const all = this.data.all;
            all[outerIndex].navList[innerIndex].compressedPath = compressedPath;
            this.setData({
                all: all
            });
        } catch (err) {
    ...
    drawCanvas: function (ctx, canvasId) {
        return new Promise((resolve, reject) => {
            ctx.draw(false, () => {
                resolve();
            });
